...
class FNMCCARDataset(Dataset):
    ...
    def __getitem__(self, itemID):
        ...
        # clip DLQ to avoid 
        np.clip(dlq, 0, 8, out=dlq) # 8 value is EOS
        # 0...6 is non def, 7 is def, and 8 is EOS
        dlq_one_hot = np.eye(9, dtype=np.float32)[dlq[:-self.predict_ahead]]
        ...
        macro_pred = macro[-self.predict_ahead:]

        #target_upb = sequence[-self.predict_ahead:, 5]
        #target = np.concatenate([target_dlq, target_upb], axis=1)

        return sequence, yymmsamod, account, macro_pred, target_dlq
...
def paddingCollator(batch):
    ...
    macro_pred_batch = [torch.from_numpy(batch[i][3]) for i in range(len(batch))]
    macro_pred_batch = torch.stack(macro_pred_batch)
    macro_pred_batch = macro_pred_batch[seq_perm_idx, :]

    target_batch = [torch.from_numpy(batch[i][4]) for i in range(len(batch))]
    target_batch = torch.stack(target_batch)
    target_batch = target_batch[seq_perm_idx, :]

    return seq_batch, seq_lengths, ymd_batch, acq_batch, macro_pred_batch, target_batch
...
if __name__ == "__main__":
    ...
    dataset = FNMCCARDataset(acq, idx_to_seq, seq, macro, ym2idx)
    ...
    for bidx, (seq, seq_len, ymd, acq, macro_pred, target) in enumerate(tqdm.tqdm(data_loader)): 
        seq = seq.to(device)
        ymd = seq.to(ymd)
        acq = acq.to(device)
        seq_len = seq_len.to(device)
        macro_pred = macro_pred.to(device)
