...
class CCARM2Model(nn.Module):
    ...
    def __init__(self, seq_n_features, lstm_size, lstm_layers, lstm_dropout, 
        ...
        """
        super(CCARM2Model, self).__init__()
        ...
        # 2*lstm_size from bidirectional
        self.bef1_lin = nn.Linear(2*lstm_size, 2*latten_size)
        self.bef1_bn  = nn.BatchNorm1d(2*latten_size)
        self.bef1_drp = nn.Dropout(0.1)

        self.bef2_lin = nn.Linear(2*latten_size, 2*latten_size)
        self.bef2_bn  = nn.BatchNorm1d(2*latten_size)
        self.bef2_drp = nn.Dropout(0.1)

        self.bef3_lin = nn.Linear(2*latten_size, 2*latten_size)
        self.bef3_bn  = nn.BatchNorm1d(2*latten_size)
        ...
        self.lstm_dec_drp1 = nn.Dropout(0.1)

        self.lstm_dec_cell2 = nn.LSTMCell(
            input_size=100, # 9 for DLQ + 14 macro vars
            hidden_size=100
        )
        self.lstm_dec_drp2 = nn.Dropout(0.1)

        self.lstm_dec_cell3 = nn.LSTMCell(
            input_size=100, # 9 for DLQ + 14 macro vars
            hidden_size=100
        )        
    ...
    def forward(self, seq, seq_len, ymd, acq, macro_pred):
        ...
        lat = torch.cat([lat[-1, 0, :, :], lat[-1, 1, :, :]], 1)
        lat = self.bef1_drp(self.bef1_bn(F.relu(self.bef1_lin(lat))))
        lat = self.bef2_drp(self.bef2_bn(F.relu(self.bef2_lin(lat))))
        lat = self.bef3_bn(F.relu(self.bef3_lin(lat)))
        ...
        h3x = torch.zeros([z.shape[0], self.lstm_dec_cell3.hidden_size], device=z.device)
        c3x = torch.zeros_like(h3x)
        ...
        for t in range(12):
            ...
            h2_in = self.lstm_dec_drp1(h1x)
            h2x, c2x = self.lstm_dec_cell2(h2_in, (h2x, c2x))
            h3_in = self.lstm_dec_drp2(h2x)
            h3x, c3x = self.lstm_dec_cell2(h3_in, (h3x, c3x))
            dlq_dist = self.bn1(F.relu(self.gen_dlq_dist1(h3x)))
...
def adjustModelPath(model_path, restart=True):
    if not restart:
        ...
        print('Reusing model path: ' + model_path)
...
if __name__ == "__main__":
    ...
    SUMMARY_PATH = '/home/user/notebooks/runs/ccarM2'
    
    # run specific model path
    MODEL_PATH = adjustModelPath('/home/user/notebooks/data/model/ccarM2/20190828_142236', True)
    print('Model will be saved in: '+MODEL_PATH)
    MODEL_SAVED = MODEL_PATH + '/ccarM2.pth'
    ...
    train_ds = FNMCCARDataset(t_acq, t_idx_to_seq, t_seq, t_macros, t_ym2idx, 12, 2)
    valid_ds = FNMCCARDataset(v_acq, v_idx_to_seq, v_seq, v_macros, v_ym2idx, 12, 2)
